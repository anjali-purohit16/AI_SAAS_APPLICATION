...
const Community = () => {
  ...
  return !loading ? (
    <div className='flex-1 min-h-screen flex flex-col text-3xl text-yellow-400 bg-linear-to-br  from-red-900 to-black gap-4 p-6 overflow-y-hidden'>
        Creations
        <div className='bg-linear-to-br from-red-900 to-black w-full rounded-xl overflow-y-hidden'>
            {creations.map((creation, index)=>(
                <div key={index} className='relative group inline-block pl-3 pt-3 w-full sm:max-w-1/2 lg:max-w-1/3'>
                    ...
                    <div className='absolute bottom-0 top-0 right-0 left-3 flex gap-2 items-end justify-end group-hover:justify-between p-3 group-hover:bg-linear-to-b from-transparent to-black/70 text-white rounded-lg'>
                        <p className='text-xl hidden group-hover:block'>{creation.prompt}</p>
